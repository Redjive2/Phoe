...
func atomBuiltins() map[string]core.StackEntry {
	return map[string]core.StackEntry{
		...
		"atomName": global(func(ctx core.Context, argv []core.Node) core.Value {
			if len(argv) != 1 {
				return ctx.Errorf(core.ErrArity, "'atomName' requires exactly 1 argument; got %d", len(argv))
			}
			v := argv[0].Evaluate(ctx)
			if v.Kind != core.KindAtom {
				return ctx.Errorf(core.ErrType, "'atomName' expected an 'atom' argument, got '%s'", v.Kind)
			}
