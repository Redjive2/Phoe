...
// sugar. Its body receives the QUOTED arguments and returns code as data,
// which the call site resumes — so (twice! n) expands to (+ n n).
func TestMacroDeclareAndCall(t *testing.T) {
	src := "(macro twice! (e) (slice '+ e e))\n(var n 5)\n(twice! n)"
	v := evalProgram(t, src)
...
// The `!` syntax is the macro-vs-function boundary at runtime: a macro called
