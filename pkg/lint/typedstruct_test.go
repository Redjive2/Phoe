...
// The typed-field struct form `(struct Name.{ T F … })` declares fields with
...
func TestTypedStructFieldsResolve(t *testing.T) {
	src := "(struct Point.{ Number x Number y })\n" +
		"(let p = Point.{ x = 1 y = 2 })\n" +
...
func TestTypedStructFieldTypeChecking(t *testing.T) {
	...
	stageE(t, []struct {
		...
		{"typed field matches param", box +
			"(fun need-n (Number) None)\n(fun need-n (n) none)\n(need-n b.n)", false},
		{"typed field clashes with param", box +
			"(fun need-s (String) None)\n(fun need-s (s) none)\n(need-s b.n)", true},
